...
graphic::cVertexBuffer g_cursor;
...
void UpdateCamera();
bool Pick(int x, int y, OUT Vector3 &out);
void GetRay(int sx, int sy, Vector3 &orig, Vector3 &dir);
bool IntersectTriangle( const D3DXVECTOR3& orig, const D3DXVECTOR3& dir,
	D3DXVECTOR3& v0, D3DXVECTOR3& v1, D3DXVECTOR3& v2,
	FLOAT* t, FLOAT* u, FLOAT* v );
void InitCursor();
void UpdateCursor(const Vector3 &cursorPos);
	...
	case WM_LBUTTONDOWN:
		{
			...
			Vector3 pos;
			Pick(g_CurPos.x, g_CurPos.y, pos);
		}
		break;
...
void Render(int timeDelta)
{
		...
		)))
	{
		...
		g_cursor.Bind();
		g_pDevice->DrawPrimitive( D3DPT_LINESTRIP, 0, g_cursor.GetVertexCount()-1);
...
void InitCursor()
{
	g_cursor.Create(64, sizeof(sVertexDiffuse), sVertexDiffuse::FVF);
}
...
void UpdateCursor(const Vector3 &cursorPos)
{
	...
	sVertexDiffuse *vertices = (sVertexDiffuse*)g_cursor.Lock();
	int index = 0;
	float angle = 0.f;
	while (index < 64)
	{
		Vector3 outter(RADIUS*cos(angle), 0.f, RADIUS*sin(angle));
		outter += cursorPos;
		vertices[ index].p = outter;
		vertices[ index].p.y = cursorPos.y;
		vertices[ index].c = D3DXCOLOR( 0, 0, 1, 0 );
